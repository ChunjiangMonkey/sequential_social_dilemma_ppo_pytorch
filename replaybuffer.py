...
class VectorizedReplayBuffer:
    ...
    def get_training_data(self):
        ...
        for key in self.buffer.keys():
            if key == 'a':
                batch[key] = torch.tensor(self.buffer[key][:, :self.train_inner_steps], dtype=torch.long)
            elif key == 'v':
                batch[key] = torch.tensor(self.buffer[key][:, :self.train_inner_steps + 1], dtype=torch.float32)
            else:
                batch[key] = torch.tensor(self.buffer[key][:, :self.train_inner_steps], dtype=torch.float32)
        return batch
