...
class MapEnv(ParallelEnv):
    def __init__(
        ...
        self.beam_pos = []
        # List:[agent objective]
        self.agents = []
        self.possible_agents = []
        # Dict: {agent id: agent objective}
        self.agents_dict = {}
    ...
    def step(self, actions):
        ...
        for agent in self.agents_dict.values():
            ...
            self.single_update_world_color_map(
                row, col, self.world_map[row, col])
        ...
        if self.inequity_averse_reward:
            assert self.num_of_agents > 1, "Cannot use inequity aversion with only one agent!"
            temp_rewards = rewards.copy()
            for agent_id in rewards.keys():
                diff = np.array([r - rewards[agent_id]
                                 for r in rewards.values()])
                dis_inequity = self.alpha * sum(diff[diff > 0])
                adv_inequity = self.beta * sum(diff[diff < 0])
                temp_rewards[agent_id] -= (dis_inequity +
                                           adv_inequity) / (self.num_of_agents - 1)
        ...
        if orientation == "UP":
            for row_elem in range(x_len):
                for col_elem in range(y_len):
                    ...
                    :] = color_map[mmap[row_elem, col_elem]]
        elif orientation == "LEFT":
            for row_elem in range(x_len):
                for col_elem in range(y_len):
                    rgb_arr[row_elem, col_elem,
                    :] = color_map[mmap[col_elem, x_len - 1 - row_elem]]
            ...
            for row_elem in range(x_len):
                for col_elem in range(y_len):
                    rgb_arr[row_elem, col_elem,
                    :] = color_map[mmap[y_len - 1 - col_elem, row_elem]]
        else:
                            ...
                            if move.tolist() in self.agent_pos:
                                ...
                                # It is trying to move into you and you are moving into it
                                    ...
                                    if (
                                            ...
                                            and move.tolist()
                                            == self.agents_dict[conflicting_agent_id].pos.tolist()
                                    ):
                                        conflict_cell_free = False
                ...
                if (
                    ...
                    for c in range(len(cell_types)):
                        if self.world_map[next_cell[0], next_cell[1]] == cell_types[c]:
                            updates.append(
                                (next_cell[0], next_cell[1], update_char[c]))
                            break
    ...
    def spawn_point(self):
        ...
        spawn_index = 0
        is_free_cell = False
        curr_agent_pos = [agent.pos.tolist()
                          for agent in self.agents_dict.values()]
